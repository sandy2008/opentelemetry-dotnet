...
/// <summary>
/// Tests for the TLS strategy factory and related components.
/// </summary>
public class OtlpTlsStrategyTests
{
    [Fact]
    public void OtlpTlsOptions_IsTlsEnabled_ReturnsFalse_WhenNoTrustedCertificatePath()
    {
    ...
    [Fact]
    public void OtlpTlsOptions_IsTlsEnabled_ReturnsTrue_WhenTrustedCertificatePathProvided()
    {
        var options = new OtlpTlsOptions { TrustedCertificatePath = "/path/to/ca.crt" };
    ...
    {
        // This is the key distinction: CA certificate alone does NOT constitute mTLS
    ...
    [Fact]
    public void OtlpMtlsOptions_CaCertificatePath_IsAliasForTrustedCertificatePath()
    {
        var options = new OtlpMtlsOptions { CaCertificatePath = "/path/to/ca.crt" };
        Assert.Equal("/path/to/ca.crt", options.TrustedCertificatePath);
        Assert.Equal("/path/to/ca.crt", options.CaCertificatePath);
    }

    [Fact]
    public void OtlpTlsStrategyFactory_ThrowsArgumentNullException_WhenOptionsIsNull()
    {
        Assert.Throws<ArgumentNullException>(() =>
            OpenTelemetryProtocol.Implementation.OtlpTlsStrategyFactory.CreateStrategy((OtlpTlsOptions)null!));
    }

    [Fact]
    public void OtlpTlsStrategyFactory_ThrowsInvalidOperationException_WhenTlsNotEnabled()
    {
        var options = new OtlpTlsOptions();
        Assert.Throws<InvalidOperationException>(() =>
            OpenTelemetryProtocol.Implementation.OtlpTlsStrategyFactory.CreateStrategy(options));
    }

    [Fact]
    public void OtlpTlsStrategyFactory_CreatesTrustedCertificateStrategy_WhenOnlyTrustedCertProvided()
    {
        RunWithCryptoSupportCheck(() =>
        {
            ...
            try
            {
                ...
                var options = new OtlpTlsOptions
                {
                    TrustedCertificatePath = tempCertFile,
                    EnableCertificateChainValidation = false,
                };

                using var strategy = OpenTelemetryProtocol.Implementation.OtlpTlsStrategyFactory.CreateStrategy(options);

                Assert.IsType<OpenTelemetryProtocol.Implementation.OtlpTrustedCertificateStrategy>(strategy);
            }
            finally
            {
    ...
    [Fact]
    public void OtlpTlsStrategyFactory_CreatesMtlsStrategy_WhenClientCertProvided()
    {
        RunWithCryptoSupportCheck(() =>
        {
            var tempCertFile = Path.GetTempFileName();
            try
            {
                ...
                Assert.IsType<OpenTelemetryProtocol.Implementation.OtlpMtlsStrategy>(strategy);
            }
            finally
            {
                if (File.Exists(tempCertFile))
    ...
    [Fact]
    public void OtlpSecureHttpClientFactory_CreatesClient_WithTrustedCertificateOnly()
    {
        RunWithCryptoSupportCheck(() =>
        {
            var tempCertFile = Path.GetTempFileName();
    ...
    [Fact]
    public void OtlpCertificateManager_LoadTrustedCertificate_ThrowsFileNotFoundException()
    {
        Assert.Throws<FileNotFoundException>(() =>
            OpenTelemetryProtocol.Implementation.OtlpCertificateManager.LoadTrustedCertificate("/nonexistent/cert.pem"));
    }

    [Fact]
    public void OtlpCertificateManager_LoadCaCertificate_IsSameAsLoadTrustedCertificate()
    {
        // LoadCaCertificate is an alias for LoadTrustedCertificate
        Assert.Throws<FileNotFoundException>(() =>
...
#pragma warning restore SYSLIB0057
    ...
    private static void RunWithCryptoSupportCheck(Action testBody)
    {
        ...
        {
            Console.WriteLine($"Skipping TLS strategy tests: {ex.Message}");
        }
